...
\[
v_i^t = \theta(t) v_i^{t-1} + \alpha \epsilon_1 [g_{n_i}^* - x_i^{t-1}] + \beta \epsilon_2 [x_i^* - x_i^{t-1}]
\],
where $\theta$ is the inertial scheduling function, $\alpha$ is the ``cognitive" component, $\beta$ is the ``social" component, $g_{n_i}^*$ is the global best position known for $i$'s neighborhood or group $n_i$, $x_i^*$ is $i$'s personal best known position, and $\epsilon_1$ and $\epsilon_2$ are randomly chosen values within $\left(0.0, 1.0\right]$. Although $\theta$ can take many forms including, most commonly, a constant function \citep[p. 101]{yang2014}, for the purposes of this paper $\theta$ is defined with respect to a base inertia $\tau$ and inertial dampening factor $\sigma$ such that
\[
